...
if [ $# -lt 1 ]; then
    echo "usage: $0 <tag> [containername]"
    exit 1
fi
...
docker run -idt --name $CONTAINER_NAME --privileged -w /shared --volume="$(pwd)":/shared/:rw miya0001/phpenv:$PHP_VERSION "/bin/bash"
...
if [ -e bin/install-wp-tests.sh ]; then
  docker exec $CONTAINER_NAME bash bin/install-wp-tests.sh wordpress_tests root 1111
fi
